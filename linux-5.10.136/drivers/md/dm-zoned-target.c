...
struct dmz_target {
        ...
        /* chunk-group mapping array */
        struct chunk_group chunk_group_mapping[1000];
...
int getMappingChunk(struct dmz_target *dmz, struct bio *bio){
	int i = 0;
	int index;

	int cgroupId = bio->bi_blkg->blkcg->css.id;
	int originCh = dmz_bio_chunk(dmz->metadata, bio);

	loop_start1:
	if( i < sizeof(dmz->chunk_group_mapping)/sizeof(dmz->chunk_group_mapping[0] )){
		struct chunk_group *cg = &dmz->chunk_group_mapping[i];
		if(cg->group == cgroupId && cg->originChunk == originCh){
			index = i;
			goto bk;	
		}else{
			i++;
			   ...
			   struct bio *bio)
{
	...
	int chunk = dmz_bio_chunk(dmz->metadata, bio);
	 ...
	 */
	zone = dmz_get_chunk_mapping(zmd, getMappingChunk(dmz,bio),
				     bio_op(bio));
	if (IS_ERR(zone)) {
		ret = PTR_ERR(zone);
		goto out;
	...
	//printk("Bio : cgroup? %d, originChunk? %d, curChunk? %d, ",
	//		css_id, chunk, getMappingChunk(dmz, bio));
	switch (bio_op(bio)) {
	case REQ_OP_READ:
		ret = dmz_handle_read(dmz, zone, bio);
		//printk("Read : cgroup? %d, zone? %u, chunk? %d", css_id, zone->id, zone->chunk);
		break;
	case REQ_OP_WRITE:
		printk("Bio Write : Cgroup? %d, chunk? %d, chunk_group? %d, zone? %d", css_id, chunk, zone->chunk, zone->id);
...
static void dmz_chunk_work(struct work_struct *work)
{
	...
	*/
	...
	unsigned int mappingChunk;
	if(bio){
		printk("Chunk %d work start", mappingChunk = getMappingChunk(dmz, bio));
		do{
			//printk("ch %d work in", mappingChunk);

			mappingChunk = getMappingChunk(dmz, bio);
			...
			//printk("ch %d work out", mappingChunk);
		}while((bio = bio_list_pop(&cw->bio_list)));
		printk("Chunk %d work finish\n", mappingChunk);
	}
	/*----------------------------------------------------------------------*/
...
static int dmz_queue_chunk_work(struct dmz_target *dmz, struct bio *bio)
{
        ...
        if( i < sizeof(dmz->chunk_group_mapping)/sizeof(dmz->chunk_group_mapping[0])){
            struct chunk_group *cg = &dmz->chunk_group_mapping[i];
            if(cg->isUsed){
                    if(cg->group == css_id && cg->originChunk == chunk){
                            goto bk;
                    }
                    else{
                            i++;
                    ...
                    cg->originChunk = chunk;
                    cg->isUsed = true;
                    cg->group = css_id;
                 /*   printk("mapping  -> isUsed? %d , group? %d, originChunk? %d, curChunk? %d",
                                    cg->isUsed, cg->group, cg->originChunk, i); */
                    goto bk;
            }
            goto loop_start1;
        }
    ...
    /*---------------------------------------------------------------------------*/
        ...
        cw = radix_tree_lookup(&dmz->chunk_rxtree, getMappingChunk(dmz, bio));
        if (cw) {
                ...
                ret = radix_tree_insert(&dmz->chunk_rxtree, i, cw);
                if (unlikely(ret)) {
                        kfree(cw);
                        goto out;
                }
